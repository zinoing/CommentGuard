...
// DEV ONLY — mock session, replace with real auth before GA
const DEV_EMAIL = "zinolubjosee@gmail.com";
const mockSession = { user: { email: DEV_EMAIL } };
...
export default function ChannelsPage() {
  ...
  const isDevUser =
    ...
    mockSession.user.email === DEV_EMAIL;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      for (const jobId of ids) {
        ...
        if (data.status === "DONE") {
          // update lastCollectedAt on matching channel
          setChannels((prev) =>
            prev.map((ch) =>
              ch.id === data.channelId ? { ...ch, lastCollectedAt: new Date().toISOString() } : ch
            )
          );
        }
      }
  ...
  const jobForChannel = (channelId: string) =>
    Object.values(activeJobs).find(
      (j) => (j as any).channelId === channelId || j.status !== "DONE"
    ) ?? null;
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", padding: "48px 0" }}>
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div className="page-head" style={{ marginBottom: 32 }}>
          <div>
            <h1>채널 선택</h1>
            <div className="sub">분석할 YouTube 채널을 선택하세요.</div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {channels.map((ch) => {
              ...
              const progress = job && job.totalVideos > 0
                ...
                : 0;
              const estMinutes = job && job.totalVideos > 0
                ? Math.ceil(((job.totalVideos - job.processedVideos) * 30) / 60)
                : null;
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 14, color: "var(--ink)" }}>{ch.name}</div>
                      ...
                      {isRunning && (
                        <div style={{ marginTop: 8 }}>
                          <div style={{ fontSize: 12, color: "var(--info-ink)", marginBottom: 4 }}>
                            초기 수집 중…
                            {estMinutes !== null && ` 예상 시간: 약 ${estMinutes}분`}
            ...
            {!inviteLink ? (
              <>
                ...
                <input
                  ...
                  style={{
                    ...
                    borderRadius: "var(--r-md)", fontSize: 14, marginBottom: 16,
                    boxSizing: "border-box",
                  }}
                />
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button className="btn" onClick={() => { setShowInviteModal(false); setInviteChannelName(""); }}>
                    취소
                  </button>
              ...
              <>
                <div style={{ fontSize: 13, color: "var(--ink-2)", marginBottom: 8 }}>
                  이 링크를 크리에이터에게 전달하세요 (7일 후 만료)
                </div>
                <div style={{
                  background: "var(--surface-2)", borderRadius: "var(--r-md)",
                  padding: "10px 12px", fontSize: 13, wordBreak: "break-all",
                  marginBottom: 12, color: "var(--ink)",
                }}>
                  {inviteLink}
                </div>
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button className="btn" onClick={() => navigator.clipboard.writeText(inviteLink)}>
                    복사
                  </button>
        ...
        <div style={{
          ...
          <div className="card" style={{ width: 480, maxWidth: "calc(100vw - 32px)" }}>
            ...
            <input
              ...
              style={{
                width: "100%", padding: "9px 12px", border: "1px solid var(--border)",
                borderRadius: "var(--r-md)", fontSize: 14, marginBottom: 12,
                boxSizing: "border-box",
              }}
            />
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button className="btn" onClick={() => { setShowDevModal(false); setDevUrl(""); setDevError(null); }}>
                취소
              </button>
